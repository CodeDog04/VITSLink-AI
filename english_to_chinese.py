...
def start_recording():
    ...
    translation = translate_to_chinese(result['text'])
    with open("output_translated.txt", "w") as f:
            f.write(result['text'].strip() + '\n')
            f.close()
    print(f"Chinese: {translation['choices'][0]['text'].strip()}")
    chinese_text = translation['choices'][0]['text'].strip()
    print("Playing Audio...")
    synthesize_and_play_audio(chinese_text)


def synthesize_and_play_audio(chinese_text):
    # Input Payload
    payload = {
        "data": [
            chinese_text,
...
# translates english text to chinese text utilizing OpenAI API
def translate_to_chinese(result):
    response = openai.Completion.create(
        engine="text-davinci-002",
        prompt=(f"Translate this text to Chinese: {result}\n\nTranslation:"),
        max_tokens=300,
        n=1,
        stop=None,
        temperature=0.5,
